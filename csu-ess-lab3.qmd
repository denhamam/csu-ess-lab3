...
## Question 4
```{r}
colorado$date <- as.Date(colorado$date)
latest_date <- max(colorado$date, na.rm = TRUE)
...
last_14_days_summary <- last_14_days %>%
  ...
  summarize(
    ...
    total_population = unique(POPESTIMATE2021), 
...
combined_data <- left_join(data, pop_data, by = fips)
